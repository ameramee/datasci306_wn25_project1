...
suggesting the value of exploring external economic or policy-related variables to better understand these fluctuations and inform future decision-making.
...
We loaded and cleaned two datasets—one detailing consumption across small consumers, industry, and power stations, and another recording Dutch natural gas prices. 
Our approach included summarizing key statistics, computing percentage changes over time, and aggregating the data to monthly means to replicate published plots. 
We standardized consumption indices to a 2015 baseline and explored data quality by identifying missingness patterns and calculating circular means for price exceedance events. 
Additionally, we created novel visualizations using advanced ggplot functions to uncover deeper insights.

Our findings reveal distinct consumption trends: industrial and power station usage notably dropped during the crisis, while household consumption, after a brief dip, rebounded strongly post-crisis. 
Supplementary analyses showed clear seasonal patterns, differences between holiday and non-holiday usage, and a strong positive relationship between solar radiation and temperature. 
These insights not only enhance our understanding of energy consumption dynamics during a crisis but also provide valuable information for policymakers and researchers focused on energy efficiency and crisis management.
